...
def train_algo_for_level(algo_name: str, level: int, cfg, device):
    env = QRBTBlockchainEnv(
    ...
    for step in trange(total_steps, desc=f"{algo_name} L{level}"):
...
def main():
    ...
    for algo in ALGORITHMS:
        print(f"\n=== Training {algo} across workload levels ===")
        all_levels_summary = {}
        for lvl in cfg["env"]["workload_levels"]:
            summary = train_algo_for_level(algo, lvl, cfg, device)
            all_levels_summary[lvl] = summary

        out_path = out_dir / f"{algo.lower()}_results.json"
        with open(out_path, "w") as f:
            json.dump(all_levels_summary, f, indent=2)
        print(f"Saved {algo} results to {out_path}")
